...
p_aw = MLNPotential(
    formula=lambda x: (np.abs(x[:, 0]) < 0.26) | (x[:, 1] != 0),
    dimension=2,
    w=2
)
...
f_dk = ParamF(p_dk, atoms=[depth('S1'), depth('S1'), seg_type('S1'), seg_type('S2')], lvs=['S1', 'S2'], subs=get_subs_matrix(dt_k_aligned))
f_dw = ParamF(p_dw, atoms=[length('S1'), depth('S1'), seg_type('S1'), seg_type('S2')], lvs=['S1', 'S2'], subs=get_subs_matrix(dt_neighbor, True))
